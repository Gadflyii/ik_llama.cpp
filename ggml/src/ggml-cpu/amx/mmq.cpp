...
void ggml_backend_amx_mul_mat(const ggml_compute_params * params, struct ggml_tensor * dst) {
    ...
    // NOTE: memset removed - AMX kernels initialize their tile accumulators with _tile_zero()
    // Output buffer zeroing is unnecessary and causes significant performance overhead
    // Upstream llama.cpp does not zero output buffers for AMX operations
    ...
    // Cast shared to threadpool - ggml_barrier will cast it back to shared
    ggml_barrier((struct ggml_threadpool *)params->shared);
    ...
    // Barrier to ensure all threads complete before checking
    ggml_barrier((struct ggml_threadpool *)params->shared);

    // Optional: Check output for NaN/Inf (debugging only - disabled in production)
    // Uncomment below to enable NaN/Inf detection for debugging:
    /*
    if (params->ith == 0) {
